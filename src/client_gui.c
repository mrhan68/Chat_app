...
void on_decline_friend_request_btn_clicked(GtkButton *btn, gpointer data)
{
    ...
    // int result = 0;
    int socket_connect = accept_pkg->ctrl_signal;
    accept_pkg->ctrl_signal = DECLINE_FRIEND_REQUEST;
    // send(socket_connect, &accept_pkg, sizeof(accept_pkg), 0);
    char str[1024];
    sprintf(str, "Sender : %s Receiver: %s Signal:%d,connect:%d", accept_pkg->sender, accept_pkg->receiver, accept_pkg->ctrl_signal, socket_connect);
    send(socket_connect, accept_pkg, sizeof(*accept_pkg), 0);
...
void show_main_window(int *client_socket_pt)
{
    ...
    group_kick_btn = GTK_WIDGET(gtk_builder_get_object(builder, "group_kick_btn"));
    ...
    g_signal_connect(group_kick_btn, "clicked", G_CALLBACK(on_group_kick_btn_clicked), client_socket_pt);
...
void show_kick_from_group_dialog(int *client_socket_pt)
{
    ...
    builder = gtk_builder_new_from_file("../views/kick_from_group_dialog.glade");

    kick_from_group_dialog = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_dialog"));
    kick_from_group_box = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_box"));
    kick_from_group_entry = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_entry"));
    kick_from_group_btn_box = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_btn_box"));
    kick_from_group_confirm_btn = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_confirm_btn"));
    kick_from_group_exit_btn = GTK_WIDGET(gtk_builder_get_object(builder, "kick_from_group_exit_btn"));

    g_signal_connect_swapped(main_window, "destroy", G_CALLBACK(gtk_widget_destroy), kick_from_group_dialog);
    g_signal_connect_swapped(kick_from_group_exit_btn, "clicked", G_CALLBACK(gtk_widget_destroy), kick_from_group_dialog);
    g_signal_connect(kick_from_group_confirm_btn, "clicked", G_CALLBACK(on_kick_from_group_confirm_btn_clicked), client_socket_pt);

    gtk_widget_show_all(kick_from_group_dialog);
}
