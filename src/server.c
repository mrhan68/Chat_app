...
void sv_user_use(int conn_socket)
{
    ...
    while (login)
    {
        ...
        case DECLINE_FRIEND_REQUEST:
            printf("IN ACCEPT_FRIEND_REQUEST\n");
            printf("Package %s %s ", pkg.sender, pkg.receiver);
            sv_decline_friend_request(conn_socket, &pkg);
            break;
...
void sv_decline_friend_request(int conn_socket, Package *pkg)
{
}
...
{
    int group_id = pkg->group_id;
    int user_id = sv_search_id_user(user,pkg->receiver);
    int i = 0;
    for (i = 0; i < MAX_USER; i++)
    {
        Member mem = group[group_id].group_member[i];
        if (strcmp(mem.username, pkg->receiver) == 0)
        {
            group[group_id].group_member[i].socket = -1;
